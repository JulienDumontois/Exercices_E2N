            ...
            <div class="col-lg-offset-1 col-lg-2 block_exo1">
                <form action="index.php" method="post">
                    ...
                    <select name="menu_mois">
                        <?php
                        $array = array(1=>'Janvier',2=>'Février',3=>'Mars',4=>'Avril',5=>'Mai',6=>'Juin',7=>'Juillet',8=>'Août',9=>'Septembre',10=>'Octobre',11=>'Novembre',12=>'Décembre');
                        foreach($array as $moisChiffres=>$moisLettres){
                            echo '<option value="' . $moisChiffres . '" ' . (($mois == $moisChiffres) ? 'selected' : '').'>'. $moisLettres .'</option>';
                        }                  
                        ?>
                    ...
                    <select name="menu_annee">
                        ...
                        for ($an = 1990; $an <= 2020; $an++) {
                            //Concaténation + ternaire : <option value= . $an . (($annee == $an) ?'selected' : '') . > . $an . </option>
                            ...
                            for ($i = 0; $i <= 6; $i++) {
                                if ($premier_jour_mois == $i) {
                                    ...
                                    for ($jour = 2; $jour <= $nb_jours; $jour++) {
                                        //Si on arrive au Samedi, on passe à la ligne
                                        if ($i == 6) {
                                            echo '</tr><tr>';
                                            //Lié à la ligne 97, afin de donner la valeur $i=0 à Dimanche
                                            $i = -1;
                                        }
                                        echo '<td>' . $jour . '</td>';
                                        $i++;
